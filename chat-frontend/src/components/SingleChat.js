...
const ENDPOINT = "http://localhost:5000"; // Replace with your endpoint after deployment
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  useEffect(() => {
    sc.on("message received", (newMessageReceived) => {
      if (
    ...
    fetch("https://api.cloudinary.com/v1_1/dbopcj4rk/image/upload", {
      ...
      .then((data) => {
        if (data.url) {
          ...
          setPic(urlString);
          setNewMessage(<a href={urlString} target="_blank" rel="noopener noreferrer">Click to download</a>);
        }
      })
      .catch((err) => {
        console.log(err);
      ...
      {selectedChat ? (
        <>
          ...
          >
            ...
            {/* Uncomment this block for video call option */}
            {/* {!selectedChat.isGroupChat && (
              <IconButton
                d={{ base: "flex", md: "none" }}
